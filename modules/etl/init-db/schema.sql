-- Schema PostgreSQL - ETL
-- Ordem: tabelas sem FK primeiro, depois tabelas com dependências
-- Idempotente: seguro rodar a cada subida do stack (CREATE IF NOT EXISTS)
...
-- Logs da aplicação (status: STARTED, IN_PROGRESS, SUCCESS, ERROR)
...
CREATE TABLE IF NOT EXISTS avisos (
    ...
    -- TODO: avaliar adicionar UNIQUE (id_evento, id_cidade, data_referencia) para
    -- garantir idempotência do load — reprocessar o mesmo dia não geraria duplicatas.
    -- Impacto: INSERT seria substituído por INSERT ... ON CONFLICT DO NOTHING em
    -- aviso_repository.py. Decidir antes de implementar a task load.
    -- Ref: documentacao/tasks.md — Idempotência do load
);
...
-- =============================================================================
-- Constraints UNIQUE (eventos e cidades para ON CONFLICT no seed)
-- =============================================================================

DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'uq_eventos_nome') THEN
        ALTER TABLE eventos ADD CONSTRAINT uq_eventos_nome UNIQUE (nome_evento);
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'uq_cidades_nome') THEN
        ALTER TABLE cidades ADD CONSTRAINT uq_cidades_nome UNIQUE (nome);
    END IF;
END $$;

-- =============================================================================
-- Índices (preferencias e FKs para consultas e joins)
-- =============================================================================
